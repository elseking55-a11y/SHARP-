...
const DcirclePage = observer(() => {
    ...
    const markets = useMemo(
        () => SUPPORTED_VOLATILITY_MARKETS.map(market => ({ label: market.label, symbol: market.symbol })),
        []
    );
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                setDigits(
                    prices
                        .map((price: unknown) => getLastDigitFromQuote(Number(price), symbol))
                        .filter((digit: number | null): digit is number => Number.isInteger(digit))
                        .slice(-tickCount)
                );
    ...
    const counts = useMemo(
        () =>
            Array.from({ length: 10 }, (_, digit) => ({
                digit,
                count: digits.filter(value => value === digit).length,
            })),
        [digits]
    );
    ...
    return (
        <main className='dcircle-page'>
            <header className='dcircle-header'>
                <div>
                    <span className='dcircle-kicker'>LIVE DIGIT MARKET</span>
                    <h1>Dcircle</h1>
                    <p>Watch the last digit move live from the selected Deriv market.</p>
                </div>
                <span className={'dcircle-status ' + (loading ? 'is-loading' : 'is-live')}>
            ...
            <section className='dcircle-board'>
                ...
                <div className='dcircle-grid'>
                    {counts.map(item => {
                        ...
                        return (
                            <div
                                key={item.digit}
                                className={'dcircle-digit ' + accentClass + (liveDigit === item.digit ? ' is-active' : '')}
                            >
                ...
                <div className='dcircle-history'>
                    {digits.slice(-30).map((digit, index) => (
                        <span key={String(index) + '-' + String(digit)} className={digit === liveDigit ? 'is-live' : ''}>
                            {digit}
                        </span>
                    ))}
                </div>
